...
- (void)viewWillLayoutSubviews {
    ...
    [self.arm setFrame:CGRectMake(0, 0, 250, 90)];
    self.arm.center = CGPointMake(.7*sWidth, .18*sHeight);
...
- (void)viewDidLoad {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"wasGameLaunched"]) {
        ...
        NSString *infoString = @"It's a perfect day to a bartender. Swipe the beer to get it moving. The length and direction of your swipe determine the speed and direction of the beer. The bar top can be slippery, so read the breaks carefully! If your slide is too fast, the beer won't find the hand of the thirsty patron even if it's on line! You get 10 attempts initially, but each successful beer passed gives you another chance. How great of a bartender can you be?";
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Bar Top" message:infoString delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        ...
        [self.ball setFrame:CGRectMake(0, 0, 100, 100)];
    ...
    self.greenSound = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:@"count"ofType:@"wav"]] error:NULL];
    ...
    self.backgroundSound = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[[NSBundle mainBundle]pathForResource:@"bar"ofType:@"mp3"]] error:NULL];
  ...
  //  if (CGRectIntersectsRect(self.ball.frame, self.image3.frame)) {
    if ((fabs(self.ball.center.x - self.image3.center.x) < horTol*self.image3.frame.size.width) && (fabs(self.ball.center.y - self.image3.center.y) < verTol*self.image3.frame.size.width)) {
        ...
        if(fabs(self.ballVelocityX) < SpeedTol && fabs(self.ballVelocityY) < SpeedTol && miss==false) {
            ...
            shots++;
            totalShots++;
            NSString *shotString = [NSString stringWithFormat:@"Beer: %i/%i", shots, totalShots];
